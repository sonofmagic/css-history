...
function App() {
  ...
  const btnClass = useMemo(
    () =>
      css`
        ...
        padding: 10px 14px;
        border-radius: 12px;
        border: 1px solid ${animatedBorder ? themes[mode].accent : themes[mode].border};
        color: ${mode === 'dark' ? '#0b1224' : '#f8fafc'};
        background: linear-gradient(135deg, ${themes[mode].accent}, ${mode === 'dark' ? '#0ea5e9' : '#0284c7'});
  ...
  return (
    <main className="page">
      <header style={{ display: 'grid', gap: '8px', marginBottom: '12px' }}>
        <p style={{ margin: 0, color: '#94a3b8', letterSpacing: '0.08em' }}>2017 · CSS-in-JS (Emotion/SC)</p>
        <h1 style={{ margin: 0, fontSize: '30px' }}>组件边界天然隔离，props 驱动样式</h1>
        <p style={{ margin: 0, color: '#cbd5e1' }}>
          Emotion 在运行时生成 class 并注入
          {' '}
          <code>&lt;style&gt;</code>
          ，主题切换与 props 状态天然绑定，但首屏注水、运行时性能需关注。
        </p>
      </header>
